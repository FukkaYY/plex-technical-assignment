...
P0〜P2およびプロフィール公開管理まで実装済みです。本番公開は対象外とし、Docker Composeによるローカル実行とGitHub Actionsによるfrontend・backend・E2E検証を提出範囲とします。
...
停止は `Ctrl+C`、コンテナの削除は `docker compose down` です。DBデータとアップロード画像も削除する場合のみ `docker compose down --volumes` を使用してください。
...
学生はマイページから企業へのプロフィール公開を停止・再開できます。非公開中は企業の学生一覧・検索・詳細から除外されますが、既存の1対1会話と面談予定は継続できます。

企業向け学生一覧では、学校種別・学校名・卒業予定年・興味のある職種を選択して絞り込めます。条件はANDで適用され、URLに保持されます。

学生は受信メッセージ詳細から、企業が開始した既存会話へ返信できます。各メッセージの送信者は学生側・企業側の双方の履歴で確認できます。

企業は学生一覧の「メッセージ一覧」から過去に連絡した学生との会話を確認できます。面談予定は15分刻みの開始日時と、15分から2時間までの所要時間を選んで提案します。

学生の受信一覧には企業から届いた未確認メッセージ数と、新しい回答待ちの面談提案件数が別々に表示されます。受信詳細を開くと表示した範囲のメッセージと面談提案が確認済みになり、学生自身の返信は未読数に含まれません。

企業は学生一覧の「募集を管理」から、任意のサムネイル付きインターン募集を作成・編集・募集終了にできます。学生はマイページの「インターン募集を見る」から公開中の募集一覧と詳細を確認し、「気になる！」の登録・解除と登録済み募集への絞り込みができます。

企業は既存の会話画面から日本時間で面談予定を提案できます。新しい提案は学生マイページと受信一覧へ赤い通知として表示され、学生は受信詳細で承諾または辞退できます。企業は回答前の提案を取り消せます。
...
現在は本番公開していません。将来公開する場合は、配置方式、HTTPS、Cookie、CORS、CSRF、秘密情報、DB migration、ログとロールバックを別途設計・検証します。
