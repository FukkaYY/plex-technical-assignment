# インターン生一覧

## 目的

企業担当者が登録済み学生の概要を比較し、詳細確認する候補を選べるようにする。

## 優先度・区分

- 優先度: P0
- 設計判断: 認証済み企業だけが閲覧できる。
- 設計判断: 新着順、1ページ20件とし、P1で検索・絞り込みを追加する。

## 表示項目

- 氏名
- 学校名
- 卒業予定年
- 興味のある職種
- スキルの一部
- 自己PRを優先したアピール情報の抜粋
- 登録日時

メールアドレスなどのログイン情報は返さない。

### 表示用の加工

- 学生を識別する `id` は `users.id` とし、詳細とメッセージ送信でも同じIDを使用する。
- スキルは登録順の先頭3件を `skills` に返し、全件数を `skills_count` に返す。
- 自己PR（移行前データは自己紹介）をRails側で最大120文字に切り詰め、120文字を超える場合だけ末尾を `…` とする。フロントエンドで追加の切り詰めを行わない。
- 登録日時 `registered_at` は `student_profiles.created_at` をUTCのISO 8601形式で返す。

## API

`GET /api/v1/students?page=1&query=Rails&graduation_year=2028&interested_role=ソフトウェアエンジニア`

- 成功: `200 OK`
- 未ログイン: `401 Unauthorized`
- 学生ロール: `403 Forbidden`
- 並び順: `student_profiles.created_at DESC, student_profiles.id DESC`
- ページサイズ: 20件固定

### P1検索・絞り込み

- `query`: 氏名、学校名、興味のある職種、スキルを対象とする100文字以内の部分一致検索。前後空白を除去し、大文字小文字を区別しない。`%`と`_`はワイルドカードではなく文字として扱う。
- `graduation_year`: 現在年から10年後までの卒業予定年に完全一致する。
- `interested_role`: 定義済みの職種1件を指定し、その職種を含むプロフィールへ絞り込む。
- 空文字は条件なしとして扱い、複数条件はANDで組み合わせる。
- 検索後も新着順と20件単位のページ分割を維持し、`meta.total_count`と`total_pages`は検索結果を基準にする。
- 不正な条件は `422 Unprocessable Entity` と対象フィールドの `invalid` エラーを返す。
- 画面は条件をURLクエリに保持し、検索実行時に1ページ目へ戻す。ページ移動時は条件を維持する。

### 成功レスポンス

```json
{
  "data": [{
    "id": 123,
    "name": "山田 太郎",
    "school_name": "プレックス大学",
    "graduation_year": 2028,
    "interested_roles": ["ソフトウェアエンジニア"],
    "skills": ["Ruby", "PostgreSQL", "Docker"],
    "skills_count": 5,
    "self_introduction_excerpt": "Webアプリの開発に取り組んでいます…",
    "registered_at": "2026-09-01T00:00:00Z"
  }],
  "meta": {
    "page": 1,
    "per_page": 20,
    "total_count": 25,
    "total_pages": 2,
    "has_previous": false,
    "has_next": true
  }
}
```

- メールアドレス、`password_digest`、`student_profiles.id` は返さない。

### ページ番号

- `page` 省略時は1ページ目を返す。
- 1以上の整数だけを受け付ける。
- 0、負数、整数以外は `422 Unprocessable Entity` と `page` / `invalid` を返す。
- 総ページ数を超える正の整数は `200 OK` と空の `data` を返す。
- 学生が0件の場合は `page=1`, `total_pages=0`, 空の `data` を返す。

## 画面状態

- 読み込み中
- 一覧表示
- 登録学生が0件の空状態
- APIエラーと再試行
- 前後ページへの移動
- 検索結果0件の空状態と条件クリア

## 画面動作

- 現在ページと総ページ数、「前へ」「次へ」を表示する。ページ番号一覧はP0では表示しない。
- APIエラー時はエラーメッセージと「再試行」を表示する。
- 未ログインは企業ログイン画面、学生ロールは学生マイページへ移動する。APIはそれぞれ `401` と `403` を返す。
- 学生カードに「詳細を見る」リンクを表示し、`/students/:id` へ移動する。
- キーワード、卒業予定年、興味のある職種を選択して検索できる。
- 学生詳細の実装前は `/students/:id` に「詳細機能は準備中」と一覧へ戻る導線を表示する。

## seedデータ

- `student01@example.com` から `student25@example.com` までの架空学生25人を作成する。
- パスワードは開発用のダミー値とし、学校名、卒業予定年、興味のある職種、スキルに複数パターンを用意する。
- seedは再実行しても同じメールアドレスのユーザーを重複作成しない。

## 受け入れ条件

- 認証済み企業が学生を新着順で閲覧できる。
- 20件単位でページ分割される。
- 学生カードから学生詳細へ移動できる。
- APIレスポンスに学生のメールアドレスが含まれない。
- 未ログインまたは学生ロールでは取得できない。
- 不正なページ番号に `422` を返し、範囲外の正のページ番号に空の一覧を返す。
- seedだけで25人の架空学生と2ページ分の表示を確認できる。
- キーワードと完全一致条件を組み合わせて学生を絞り込める。
- 検索結果0件を通常の登録学生0件と区別し、条件をクリアできる。
- URLの検索条件を再読み込み後も復元でき、ページ移動後も維持できる。
